...
type Video struct {
	...
	Video_name  string          `json:"video_name" gorm:"not null;type:varchar(100)" validate:"required,max=100"`                   // Video_name is the name of the video
	Price       int32           `json:"price" gorm:"not null;type:int" validate:"required,number"`                                  // Price is the price of the video
	Picture     string          `json:"picture" gorm:"not null;type:varchar(255)" validate:"required,max=100"`                      // Picture is a link to the picture
	...
	Url         string          `json:"url" gorm:"not null;type:varchar(255)" validate:"required,max=255"`                          // link file
	Sheet       string          `json:"sheet" gorm:"not null;type:varchar(255)" validate:"required,max=255"`                        // link file
