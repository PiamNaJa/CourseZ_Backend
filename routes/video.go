...
func VideoRoutes(app fiber.Router, db *gorm.DB) {
	...
	app.Get("/:video_id/islike", m.IsLogin, handlers.IsLikeVideo(db))
	app.Patch("/:video_id/like", m.IsLogin, m.IsTeacher, handlers.LikeVideo(db))
	app.Delete("/:video_id", m.IsLogin, m.IsTeacher, m.IsVideoOwner, handlers.DeleteVideoByID(db))
	app.Put("/:video_id", m.IsLogin, m.IsTeacher, m.IsVideoOwner, handlers.UpdateVideo(db))
}
